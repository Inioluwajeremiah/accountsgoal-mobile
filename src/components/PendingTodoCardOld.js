...
const PendingTodoCard = ({ item, refetch }) => {
  ...
  const [toggleReminder, setToggleReminder] = useState(item.setReminder.active);
    ...
    >
      ...
      {selectedItem.includes(item._id) && (
        <View className="flex flex-row items-center pl-4">
          {item.status === false && (
            <EditComponent onPressEdit={handleToggleEditTodoModal} />
          )}
          {loadingDelete ? (
